...
echo ""
micromamba env create --name "$conda_env_name" --file "$conda_env_path"

echo ""
echo "Adding pydot_ng requirement via pip..."
echo ""
micromamba run -n SDSD-phyddle python -m pip install pydot_ng
...
then
    (
        ...
        micromamba run -n SDSD-phyddle python -m pip install -e .
    )
else
    echo ""
