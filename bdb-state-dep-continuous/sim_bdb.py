...
class Priors(object):
    def __init__(
        self,
        state_rate_prop = st.uniform(0.1, 0.6),
        log_birth_rate = st.uniform(-2.0, 2.0),
        log_death_rate_prop = st.uniform(-2.0, 2.0),
        burst_rate_prop = st.uniform(0.3, 0.9),
        burst_prob_0 = st.uniform(0.65, 0.35),
        burst_prob_1 = st.uniform(0.10, 0.55),
        # model = st.randint(0, 2),
        root_state = st.randint(0, 2),
    ):
        self.state_rate_prop = state_rate_prop
        self.log_birth_rate = log_birth_rate
        self.log_death_rate_prop = log_death_rate_prop
        self.burst_rate_prop = burst_rate_prop
        self.burst_prob_0 = burst_prob_0
        self.burst_prob_1 = burst_prob_1
        # self.model = model 
...
def draw_bdb_model(priors, rng):
    ...
    state_rate_mult = priors.state_rate_prop.rvs(size = None, random_state = rng)
    state_rate = min([birth_rate_0, birth_rate_1]) * state_rate_mult
    
    burst_prob_0 = priors.burst_prob_0.rvs(size = None, random_state = rng)
    burst_prob_1 = priors.burst_prob_1.rvs(size = None, random_state = rng)

    root_state = priors.root_state.rvs(size = None, random_state = rng)
    ...
    model = sdsdsim.model.SDSDModel(
        ...
        burst_probs = [burst_prob_0, burst_prob_1],
...
def get_model_parameters(sdsd_model):
    return {
        ...
        'expected_burst_rate_diff' : (
            (sdsd_model.burst_rate * sdsd_model.burst_probs[0])
            - (sdsd_model.burst_rate * sdsd_model.burst_probs[1])
        ),
    }
...
def main():
    ...
    label_names = [
        ...
        'log10_expected_burst_rate_diff',
    ]
    ...
    while sim_idx < (start_idx + batch_size):
        ...
        model, root_state = draw_bdb_model(priors, rng)
        ...
        # reduced_log10_variables['model_type'] = model_type
        reduced_log10_variables['root_state'] = root_state 
        write_variables(labels_path, reduced_log10_variables)
        write_tip_states(data_path, root, all_zeros = False)
